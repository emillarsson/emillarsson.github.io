...
function SetTooltip(title, price, desc) { 
    ...
    if (price > 0) {
        tooltipPrice.innerHTML = Math.ceil(price).toLocaleString();
    } else {
...
new Game.Ingredient('Water', 'Refreshing.', 0, null, function() {this.noItems+=Game.clickAmount;if (this.noItems >= 20) Game.Items['Wheat'].unlock();});

new Game.Ingredient('Wheat', 'Wheat the fuck.', 0, function(seconds) {
    this.ps=Game.Items['Farmer'].pps;
    this.noItems+=this.ps*seconds;}, function() {this.noItems+=Game.clickAmount;if (this.noItems >= 20) Game.Upgrades['Wheat Dough'].unlock();});
...
var moon = function(seed, planetSize, id) {
    this.id = id;
    var r = Math.seededRandom(6,12,seed);
    this.size = r.number;
    r = Math.seededRandom(40,1000,r.seed);
    this.orbit = r.number;
    this.velocity = 3*Math.sqrt(planetSize)/this.orbit;
    ...
    this.checkBoundary = function(x,y) {
        var pos = [canvas.width/2+drawScale*this.orbit*Math.cos(this.offset),
                       canvas.height/2+drawScale*this.orbit*Math.sin(this.offset)];
        if (Math.sqrt(Math.pow(x-pos[0],2)+Math.pow(y-pos[1],2)) <= this.size*4) {
            SetTooltip('Moon: ' + this.id, '', '');
            return true;
        }
    }
}

var planet = function(seed, sunSize, id) {
    this.id = id;
    ...
    for (var i = 0; i < this.noMoons; i++) {
        this.moons[i] = new moon(Game.planetSeed, this.size, i);
    ...
    this.orbit = r.number;
    this.velocity = 3*Math.sqrt(sunSize)/this.orbit;
    r = Math.seededRandom(0,2*Math.PI,r.seed);
    this.startOffset = r.number;
    this.offset = this.startOffset;
    ...
    this.checkBoundary = function(x,y) {
        if (Game.currentPlanet-1==this.id) {
            for (var i = 0; i < this.noMoons; i++) {
                if (this.moons[i].checkBoundary(x,y)) {
                    return true;
                }
            }
        } else {
...
var solarSystem = function(seed) {
    ...
    for (var i = 0; i < this.noPlanets; i++) {
        this.planets[i] = new planet(Game.planetSeed, this.sun.size, i);
    ...
    this.checkBoundary = function(x,y) {
        ...
        if (Game.currentPlanet > 0) {
            found = this.planets[Game.currentPlanet-1].checkBoundary(x,y);
        } else {
            for (var i = 0; i < this.noPlanets; i++) {
                if (this.planets[i].checkBoundary(x,y)) {
                    found = true;
                    break;
                }
            }
...
window.onmousemove=function(e) {
    if (e.target.className == 'upgrade') {
        ...
        solarSystemsById[Game.currentSolarSystem].checkBoundary(e.layerX,e.layerY);
    } else {
        SetTooltip('','','');
        //disableDisplay('tooltip');
    }
...
window.onclick = function(e) {
    if (e.target.className == 'containerButton') {
        ...
        Game.currentPlanet++;
        if (Game.currentPlanet > solarSystemsById[Game.currentSolarSystem].noPlanets+1) {
